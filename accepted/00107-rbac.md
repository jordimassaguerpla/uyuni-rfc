...
This RFC defines role-based access control (RBAC) in Uyuni to meet users' needs for fine-grained access control and configuration.
...
Access control is enforced by defining rules for these namespaces through **access groups**.
...
Users can belong to multiple access groups and inherit the union of all associated access rules. Permissions are additive; there are no explicit deny rules.
...
Below is a conceptual example of how access rules can be defined for a custom access group:

**Example:** Grant tailored access to the content management (images) feature for the `Image Managers` access group
...
Setting up access rules for the `Image Managers` access group:

- Grant view access to the entire images feature:

  ```
  Grant 'View' on 'cm.*' to 'Image Managers'
  ```

- Prevent access to sensitive image store information:

  ```
  Revoke 'View' on 'cm.store.details' from 'Image Managers'
  ```

- Allow `Alice` to build and manage images:

  ```
  Grant 'All' on 'cm.build' to 'Image Managers'
  Grant 'All' on 'cm.image.*' to 'Image Managers'
  ```

- Users assigned to `Image Managers` can now:
...
- When permissions are no longer needed, revoke them:
  ```
  Revoke 'All' on 'cm.*' from 'Image Managers'
  ```
...
ACL methods can check access to arbitrary namespaces when controllers need additional authorization checks or conditional rendering.

The filter controls access to the following types of endpoints:

- Struts pages
- Spark endpoints, including web UI pages, internal and public APIs, and Saltboot
- Ajax endpoints

By default, any requested URI is denied access unless a match is found.

Websocket endpoints perform explicit namespace checks when authorization beyond an authenticated session is required.
...
A management UI allows administrators to create custom access groups, browse and search the namespace catalog, assign View and Modify permissions, and manage group membership.

Administrators can also manage access through the XML-RPC API namespace `access`, which provides methods to:

- Create, list, and delete custom access groups, exposed as roles in the API
- Grant or revoke namespace permissions for a custom access group
- List available namespaces and the permissions assigned to a group

User membership is managed through the existing user role API.
